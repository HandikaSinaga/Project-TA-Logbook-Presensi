...
import { Card, Badge, Spinner, Alert, Row, Col, Button, Form, InputGroup } from "react-bootstrap";
import axiosInstance from "../../utils/axiosInstance";
import toast from "react-hot-toast";
import { isFutureDate, isToday as checkIsToday, formatDateToString } from "../../utils/dateHelper";
...
const UserWorkCalendar = () => {
    ...
    const generateEvents = useCallback((data) => {
        ...
        if (data.period && data.workingDays) {
            ...
            const attendanceDates = new Set(data.attendances?.map(att => att.date) || []);
            ...
            data.leaves?.forEach((leave) => {
                ...
                for (let d = leaveStart.clone(); d.isSameOrBefore(leaveEnd); d.add(1, 'day')) {
                    leaveDates.add(d.format('YYYY-MM-DD'));
                }
            });
            
            // Check each working day (only past dates and today)
            for (let date = startDate.clone(); date.isSameOrBefore(endDate); date.add(1, 'day')) {
                ...
                const hasHoliday = eventsByDate[dateStr]?.some(e => e.type === 'holiday');
                const hasAttendance = attendanceDates.has(dateStr);
                const hasLeave = leaveDates.has(dateStr);
                const isFuture = date.isAfter(today);
                
                // Mark as absent if: NOT WEEKEND, working day, no holiday, no attendance, no leave, NOT FUTURE
                if (!isWeekend && isWorkingDay && !hasHoliday && !hasAttendance && !hasLeave && !isFuture) {
                    addEvent(dateStr, {
                        id: `absent-${dateStr}`,
                        title: "❌ Alpha",
    ...
    const fetchDateDetail = useCallback(async (date) => {
        try {
            ...
            const response = await axiosInstance.get(`/user/calendar/date/${dateStr}`);
                    ...
                    <Col xs={6} md={3}>
                        <Card className="border-0 shadow-sm h-100" style={{
                            ...
                            <Card.Body className="p-3 text-center">
                                ...
                                <h3 className="mb-1 fw-bold">{calendarData.summary.totalAttendances}</h3>
                                <small className="text-white-50 fw-medium">Hadir</small>
                            </Card.Body>
                        </Card>
                    </Col>
                    <Col xs={6} md={3}>
                        <Card className="border-0 shadow-sm h-100" style={{
                            ...
                            <Card.Body className="p-3 text-center">
                                ...
                                <h3 className="mb-1 fw-bold">{calendarData.summary.lateCount}</h3>
                                <small className="text-white-50 fw-medium">Telat</small>
                            </Card.Body>
                        </Card>
                    </Col>
                    <Col xs={6} md={3}>
                        <Card className="border-0 shadow-sm h-100" style={{
                            ...
                            <Card.Body className="p-3 text-center">
                                ...
                                <h3 className="mb-1 fw-bold">{calendarData.summary.absentCount || 0}</h3>
                                <small className="text-white-50 fw-medium">Alpha (Hari)</small>
                            </Card.Body>
                        </Card>
                    </Col>
                    <Col xs={6} md={3}>
                        <Card className="border-0 shadow-sm h-100" style={{
                            ...
                            <Card.Body className="p-3 text-center">
                                ...
                                <h3 className="mb-1 fw-bold">{calendarData.summary.leaveCount || 0}</h3>
                                <small className="text-white-50 fw-medium">Cuti (Hari)</small>
                            </Card.Body>
                        </Card>
                    </Col>
                </Row>
            ...
            <Card className="mb-4 border-0" style={{
                ...
                <Card.Body className="p-4">
                    <div className="d-flex align-items-center mb-3">
                        ...
                        <div>
                            <h6 className="mb-0 fw-bold">Keterangan Warna Kalender</h6>
                            <small className="text-muted">Sistem prioritas untuk menampilkan status</small>
                        </div>
                    </div>
                    ...
                    <Row className="g-3">
                        {[
                            ...
                            { color: COLORS.absent, label: "Tidak Masuk (Alpha)", icon: "❌", priority: 3 },
                            { color: COLORS.leave, label: "Izin/Cuti Disetujui", icon: "🏖️", priority: 4 },
                            { color: COLORS.late, label: "Terlambat", icon: "⏰", priority: 5 },
                            { color: COLORS.present, label: "Hadir Tepat Waktu", icon: "✓", priority: 6 },
                        ].map((item) => (
                            <Col xs={6} md={4} lg={2} key={item.priority}>
                                ...
                                >
                                    ...
                                    <div className="flex-grow-1">
                                        ...
                                        <small className="text-muted" style={{ fontSize: '0.7rem' }}>Prioritas {item.priority}</small>
                                    </div>
                                </div>
                            </Col>
                        ))}
                    ...
                    <Alert variant="info" className="mt-4 mb-0 border-0" style={{
                        background: 'linear-gradient(135deg, #667eea15 0%, #764ba215 100%)',
                        borderLeft: '4px solid #667eea'
                    }}>
                        <div className="d-flex align-items-start">
                            <i className="bi bi-lightbulb me-2 text-primary" style={{ fontSize: '1.2rem' }}></i>
                            <div>
                                <strong className="text-primary">Tips:</strong>
                                <span className="ms-1">Jika dalam satu hari terdapat beberapa status, kalender akan menampilkan status dengan prioritas tertinggi. Tanggal di masa depan tidak dapat diklik.</span>
                            </div>
                        </div>
                    </Alert>
                </Card.Body>
                    ...
                    <Row className="align-items-center g-2">
                        <Col md={6}>
                            <div className="d-flex align-items-center">
                                ...
                                <div>
                                    <h5 className="mb-0 fw-bold">Tampilan Kalender</h5>
                                    <small className="text-white-50">Pilih periode untuk melihat aktivitas</small>
                                </div>
                            </div>
                        </Col>
                        <Col md={6}>
                                            ...
                                            {Array.from({ length: 12 }, (_, i) => {
                                                ...
                                                return (
                                                    <option key={month} value={month}>
                                                        {moment(`2024-${month}-01`).format('MMMM')}
                                                    </option>
                                                );
                                            })}
                                        </Form.Select>
                                        ...
                                        {dateDetail.holiday ? (
                                            <div className="mt-2">
                                                ...
                                                <div className="mt-2 ps-2 border-start border-danger border-3">
                                                    ...
                                                    <div className="text-muted small mt-1">
                                                        Tipe: <span className="badge bg-danger-subtle text-danger">
                                                            {dateDetail.holiday.is_national ? "Libur Nasional" : "Hari Libur"}
                                                        </span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <Badge bg="secondary" className="ms-2">📅 Akhir Pekan</Badge>
                                        )}
                                    </Alert>
                                    ...
                                    <Row className="g-3">
                                                    ...
                                                    <Card.Body className="p-3">
                                                        <Row className="mb-2">
                                                            <Col xs={6}>
                                                                ...
                                                                <strong>{dateDetail.attendance.check_in_time || "-"}</strong>
                                                            </Col>
                                                            <Col xs={6}>
                                                                <small className="text-muted d-block">Check Out</small>
                                                                <strong>{dateDetail.attendance.check_out_time || "-"}</strong>
                                                            </Col>
                                                        </Row>
                                                        <div>
                                                            {dateDetail.attendance.status === 'late' && (
                                                                <Badge bg="warning" text="dark" className="me-1">⏰ Terlambat</Badge>
                                                            )}
                                                            {dateDetail.attendance.work_type === "offsite" && (
                                                                <Badge bg="info">📍 Offsite</Badge>
                                                            )}
                                                        </div>
                                                        {dateDetail.attendance.notes && (
                                                            <div className="mt-2 pt-2 border-top">
                                                                <small className="text-muted d-block">Catatan:</small>
                                                                <small>{dateDetail.attendance.notes}</small>
                                                            </div>
                                                        )}
                                                    </Card.Body>
                                                </Card>
                                        ...
                                        <Col md={6}>
                                            ...
                                            {dateDetail.logbook ? (
                                                <Card className="border">
                                                    <Card.Body className="p-3">
                                                        <h6 className="mb-1">{dateDetail.logbook.activity}</h6>
                                                        <p className="text-muted small mb-0">
                                                            {dateDetail.logbook.description}
                                                        </p>
                                                    </Card.Body>
                                                </Card>
                                            ) : (
                                                ...
                                                <Card className="border">
                                                    <Card.Body className="p-3">
                                                        <div className="d-flex justify-content-between align-items-start">
                                                            <div>
                                                                <h6 className="mb-1">
                                                                    {dateDetail.leave.type.replace("izin_", "").toUpperCase()}
                                                                </h6>
                                                        ...
                                                        {dateDetail.leave.supervisor && (
                                                            <p className="mb-0 small text-muted mt-2">
                                                                Disetujui oleh: {dateDetail.leave.supervisor.name}
                                                            </p>
                                                        )}
                                                    </Card.Body>
                                                </Card>
